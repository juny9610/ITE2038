...
class BPlusTree:
            ...
            if isinstance(node, LonelyRootNode): #Root Node & Leaf Node
                ...
                if node.can_delete_entry:
                    ...
                    if node.smallest_key > parent.biggest_key:
                        assert("Tree Error, parent's biggest key < child's smallest key")
    ...
    def _redistribute_leaf(self, src_node, dst_node):
        ...
        dst_node.insert_entry(src_node.pop_smallest())

        for entry in parent.entries:
            if entry.before == src_node.page or entry.after == src_node.page:
                ref = self.Reference(src_node.smallest_entry.key, dst_node.page, src_node.page)
                parent.entries[parent._find_entry_index(entry.key)] = ref
                break
    ...
    def _redistribute_parent(self, src_node, dst_node):
        ...
        for entry in parent.entries:
            if entry.before == src_node.page or entry.after == src_node.page:
                ref = self.Reference(src_node.smallest_entry_key, dst_node.page, src_node.page)
                parent.entries[parent._find_entry_index(entry.key)] = ref
                break
    ...
    def _merge_leaf(self, src_node, dst_node):
        ...
        # To do
        if src_node.next_page == dst_node.page:
            tmp = dst_node.page
            dst_node.page = src_node.page
            src_node.page = tmp

        for entry in src_node.entries:
            dst_node.insert_entry(entry)
        dst_node.next_page = src_node.next_page
        ...
        if isinstance(parent, RootNode):
            if parent.can_delete_entry:
                ...
                for entry in parent.entries:
                    if entry.before == src_node.page or entry.after == src_node.page:
                        ref = self.Reference(entry.key, dst_node.biggest_entry.after, src_node.smallest_entry.before)
                        dst_node.insert_entry(ref)
                    break

                for entry in src_node.entries:
                    dst_node.insert_entry(entry)

                parent = parent.convert_to_internal()
                dst_node = dst_node.conver_to_root()
                self._root_node_page = dst_node.page
            ...
            for entry in grandParent.entries:
                if entry.before == parent.page:
                    src_parent = self._mem.get_node(entry.after)
                    isReverse = True
                    break

            if not isReverse:
                for entry in grandParent.entries:
                    if entry.after == parent.page:
                        src_parent = self._mem.get_node(entry.before)
                        break
    ...
    def _merge_parent(self, src_node, dst_node):
        ...
        # To do
        new_node = self.InternalNode(page = dst_node.page)
        if src_node.next_page == dst_node.page:
            tmp = dst_node.page
            dst_node.page = src_node.page
            src_node.page = tmp
        ...
        if isinstance(parent, RootNode):
            if parent.can_delete_entry:
                # To do
                for entry in parent.entries:
                    if entry.before == dst_node.page or entry.after == dst_node.page:
                        parent.remove_entry(entry.key)
                        ref = self.Reference(entry.key, src_node.biggest_entry.after, dst_node.smallest_entry.before)
                        src_node.insert_entry(ref)
                        break

                for entry in dst_node.entries:
                    src_node.insert_entry(entry)
                ...
                for entry in src_node.entries:
                    dst_node.insert_entry(entry)
                parent = parent.convert_to_internal()
                dst_node = dst_node.convert_to_root()
                self._root_node_page = dst_node.page
                self._mem.set_node(dst_node)
                print("Root Node Delete")
